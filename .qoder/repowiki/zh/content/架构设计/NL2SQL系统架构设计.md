...
核心引擎实现了完整的自然语言到SQL转换流程，包含意图识别、澄清机制、SQL生成、验证和结果格式化等关键步骤。
...
### 6. Schema管理器 (schemaLoader.js)

Schema加载器负责管理数据表的元数据信息，提供Schema查询、匹配和验证功能。

**章节来源**
- [schemaLoader.js:1-655](file://NL2SQL/backend/src/core/schemaLoader.js#L1-L655)

### 7. WebSocket处理器 (wsHandler.js)
