...
- **实时会话管理**：支持多会话创建、切换和历史记录管理，具备自动刷新机制
...
**更新** 聊天界面实现了精确的键盘事件处理机制，提供更好的用户体验：

- **Enter键发送**：当用户按下Enter键时，系统阻止默认行为（防止插入换行符），然后调用sendMessage()方法发送消息
- **Shift+Enter换行**：当用户按下Shift+Enter组合键时，允许正常的换行行为
- **输入验证**：在发送前检查输入内容是否为空，以及是否正在处理其他请求
- **自动滚动**：发送消息后自动滚动到消息列表底部

```mermaid
flowchart TD
A[用户按下键盘] --> B{检测按键}
B --> |Enter| C{检查Shift键}
C --> |Shift已按下| D[允许默认换行行为]
C --> |Shift未按下| E[阻止默认行为]
E --> F[调用sendMessage()]
B --> |其他键| G[正常输入处理]
F --> H[消息发送流程]
...
**图表来源**
- [ChatView.vue:220-229](file://NL2SQL/frontend/src/views/ChatView.vue#L220-L229)
